import { describe, it, expect } from 'vitest';
import { LocalRandomCodemaker } from '../LocalRandomCodemaker';
import { isValidCode } from '../codeGenerator';
import { judge } from '../judge';
import { DEFAULT_CONFIG } from '../types';
import { mulberry32, seededRng } from './helpers';

describe('LocalRandomCodemaker', () => {
  it('startGame 後 reveal() 回傳一組合法答案', () => {
    const maker = new LocalRandomCodemaker();
    maker.startGame(DEFAULT_CONFIG);
    expect(isValidCode(maker.reveal(), DEFAULT_CONFIG)).toBe(true);
  });

  it('注入 rng 時答案可預期', () => {
    const maker = new LocalRandomCodemaker(seededRng([0]));
    maker.startGame(DEFAULT_CONFIG);
    expect(maker.reveal()).toBe('0123');
  });

  it('尚未 startGame 就呼叫 judge 或 reveal 會拋出錯誤', () => {
    const maker = new LocalRandomCodemaker();
    expect(() => maker.reveal()).toThrow();
    expect(() => maker.judge('1234')).toThrow();
  });

  it('judge 的結果與純函式 judge(reveal(), guess) 一致', () => {
    const maker = new LocalRandomCodemaker(mulberry32(7));
    maker.startGame(DEFAULT_CONFIG);
    const answer = maker.reveal();
    for (const guess of ['0123', '4567', '8901', '1234', answer]) {
      expect(maker.judge(guess)).toEqual(judge(answer, guess));
    }
  });

  it('猜中答案時回傳 4A0B', () => {
    const maker = new LocalRandomCodemaker(seededRng([0]));
    maker.startGame(DEFAULT_CONFIG);
    expect(maker.judge(maker.reveal())).toEqual({ A: 4, B: 0 });
  });

  it('judge 收到不合法的猜測會拋出錯誤', () => {
    const maker = new LocalRandomCodemaker();
    maker.startGame(DEFAULT_CONFIG);
    expect(() => maker.judge('112')).toThrow();
    expect(() => maker.judge('1123')).toThrow();
    expect(() => maker.judge('12a4')).toThrow();
  });

  it('重新 startGame 會換一組新答案', () => {
    const maker = new LocalRandomCodemaker(mulberry32(99));
    const seen = new Set<string>();
    for (let i = 0; i < 200; i += 1) {
      maker.startGame(DEFAULT_CONFIG);
      seen.add(maker.reveal());
    }
    expect(seen.size).toBeGreaterThan(1);
  });

  it('答案在整局之中保持固定（本次不是惡魔模式）', () => {
    const maker = new LocalRandomCodemaker(mulberry32(3));
    maker.startGame(DEFAULT_CONFIG);
    const answer = maker.reveal();
    for (const guess of ['0123', '4567', '8912', '3456']) maker.judge(guess);
    expect(maker.reveal()).toBe(answer);
  });

  it('兩個獨立實例互不干擾', () => {
    const a = new LocalRandomCodemaker(seededRng([0]));
    const b = new LocalRandomCodemaker(seededRng([0.999999]));
    a.startGame(DEFAULT_CONFIG);
    b.startGame(DEFAULT_CONFIG);
    expect(a.reveal()).toBe('0123');
    expect(b.reveal()).toBe('9012');

    b.startGame(DEFAULT_CONFIG);
    expect(a.reveal()).toBe('0123');
  });

  it('答案存於私有欄位，無法從實例外部列舉出來', () => {
    const maker = new LocalRandomCodemaker(seededRng([0]));
    maker.startGame(DEFAULT_CONFIG);
    expect(Object.keys(maker)).toEqual([]);
    expect(JSON.stringify(maker)).toBe('{}');
  });
});
